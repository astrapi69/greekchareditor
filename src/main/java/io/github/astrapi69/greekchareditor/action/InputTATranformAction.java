...
import io.github.astrapi69.greekchareditor.util.GreekAlphabetUtil;
...
public class InputTATranformAction extends AbstractAction
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		JTextArea textAreaInput = applicationPanel.getJtaInput();
		JTextArea textAreaPreview = applicationPanel.getJtaPreview();
		JTextArea textAreaHtmlEntitys = applicationPanel.getJtaHtmlEntitys();
		JTextArea textAreaISO_8859_7 = applicationPanel.getJtaIso8859_7();
		JTextArea textAreaUnicode = applicationPanel.getJtaUnicode();
		transformLatin2HtmlEntities(textAreaInput, textAreaPreview, textAreaHtmlEntitys,
			textAreaISO_8859_7, textAreaUnicode);
	}
	 ...
	 */
	private void transformLatin2HtmlEntities(JTextArea textAreaInput, JTextArea textAreaPreview,
		JTextArea textAreaHtmlEntitys, JTextArea textAreaISO_8859_7, JTextArea textAreaUnicode)
	{
		...
		String buffer = EMPTYSTRING, underscore = UNDERSCORE, doppelpunkt = COLON,
			combi = COLONUNDERSCORE;
		...
		for (int i = 0; i < length; i++)
		{
			...
			{ // than we must control the char
				...
				{ // otherwise
					...
					if (latin2Greek.containsKey(EMPTYSTRING + current))
					{ // than
						// append the transformed character to each stringbuffer
						outputGreekChars.append(latin2Greek.getProperty(EMPTYSTRING + current));
						outputhtmlChars.append(latin2Gr4HTML.getProperty(EMPTYSTRING + current));
						outputISO_8859_7
							.append(latin2ISO_8859_7.getProperty(EMPTYSTRING + current));
					}
					else
					{ // otherwise
		...
		textAreaUnicode
			.setText(StringExtensions.toUnicodeChars(outputGreekChars.toString(), false));
	}
